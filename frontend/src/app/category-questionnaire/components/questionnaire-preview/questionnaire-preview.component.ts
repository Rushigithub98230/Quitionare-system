...
import { UserQuestionResponseService } from '../../services/user-question-response.service';
...
export class QuestionnairePreviewComponent implements OnInit {
  ...
  initializeForms(): void {
    console.log('Initializing forms for', this.questions.length, 'questions');
    this.questionForms = this.questions.map(question => {
      ...
      if (question.questionTypeName === 'slider') {
        // For slider, initialize with the minimum value
        const minValue = question.minValue && question.minValue > 0 ? question.minValue : 1;
        defaultValue = minValue;
        console.log(`Initializing slider for question ${question.id}: min=${minValue}, max=${question.maxValue || 10}`);
      } else if (question.questionTypeName === 'number') {
        // For number inputs, initialize with null to allow empty state
        defaultValue = null;
      } else if (question.questionTypeName === 'yes_no') {
        // For yes/no questions, initialize with empty string
        defaultValue = '';
      } else if (question.questionTypeName === 'rating') {
        // For rating questions, initialize with null
        defaultValue = null;
      }
    ...
    console.log('Forms initialized:', this.questionForms.length);
    // Debug: Log slider forms specifically
    this.questionForms.forEach((form, index) => {
      const question = this.questions[index];
      if (question.questionTypeName === 'slider') {
        console.log(`Slider form ${index}:`, {
          questionText: question.questionText,
          defaultValue: form.get('response')?.value,
  ...
  submitPreview(): void {
    ...
    // Collect all responses with enhanced formatting
    this.responses = this.questions.map((question, index) => {
      const form = this.getQuestionForm(index);
      const response = form.get('response')?.value;
      
      // Enhance response with option text for better user experience
      let enhancedResponse = response;
      
      if (response && (question.questionTypeName === 'radio' || question.questionTypeName === 'checkbox' || question.questionTypeName === 'select' || question.questionTypeName === 'multiselect')) {
        enhancedResponse = this.enhanceResponseWithOptionText(response, question);
      }
  ...
  onFileSelected(event: any, questionIndex: number): void {
    ...
    if (file) {
      ...
      form.get('response')?.setValue(null);
    }
  }
  ...
  onSliderChange(event: any, questionIndex: number): void {
    const form = this.getQuestionForm(questionIndex);
    const value = event.value || event.target?.value;
    form.get('response')?.setValue(value);
    console.log(`Slider changed for question ${questionIndex}: ${value}`);
    
    // Force change detection for the form
    form.markAsTouched();
    form.updateValueAndValidity();
  }

  private enhanceResponseWithOptionText(response: any, question: Question): any {
    if (!response || !question.options || question.options.length === 0) {
      return response;
    }

    // For single selection (radio, select)
    if (question.questionTypeName === 'radio' || question.questionTypeName === 'select') {
      const selectedOption = question.options.find(option => 
        option.id === response || option.optionValue === response
      );
      ...
      if (selectedOption) {
        return {
          ...
          displayText: selectedOption.optionText
        };
      }
    }
    
    // For multiple selection (checkbox, multiselect)
    if (question.questionTypeName === 'checkbox' || question.questionTypeName === 'multiselect') {
      if (Array.isArray(response)) {
        const selectedOptions = question.options.filter(option => 
          response.includes(option.id) || response.includes(option.optionValue)
        );
        
        return {
          ids: selectedOptions.map(option => option.id),
          values: selectedOptions.map(option => option.optionValue),
          texts: selectedOptions.map(option => option.optionText),
          displayText: selectedOptions.map(option => option.optionText).join(', ')
        };
      }
    }
