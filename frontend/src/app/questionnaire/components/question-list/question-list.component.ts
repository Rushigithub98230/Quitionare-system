...
import { QuestionDialogComponent } from '../../questionnaire.module';
...
export class QuestionListComponent implements OnInit, OnDestroy {
  ...
  loadQuestionTypes(): void {
    this.questionService.getQuestionTypes().pipe(
  ...
  loadQuestions(): void {
    ...
    this.loading = true;
    this.questionService.getQuestionsByQuestionnaireId(this.questionnaire.id).pipe(
      takeUntil(this.destroy$)
    ).subscribe({
      next: (response: any) => {
  ...
  saveQuestionOrder(): void {
    ...
    this.questionService.updateQuestionOrder(this.questionnaire.id, this.questions).pipe(
      takeUntil(this.destroy$)
    ).subscribe({
